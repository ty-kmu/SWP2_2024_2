...
#define _EMA_ALPHA \
    0.47  // EMA weight of new sample (range: 0 to 1)
          // Setting EMA to 1 effectively disables EMA filter

// Servo adjustment - Set _DUTY_MAX, _NEU, _MIN with your own numbers
#define _DUTY_MAX 2100  // 2000
#define _DUTY_NEU 1553  // 1500
#define _DUTY_MIN 1100  // 1000

#define _SERVO_ANGLE_DIFF 276  // Replace with |D - E| degree
#define _SERVO_SPEED 400       // servo speed

// PID parameters
// #define _KP 2.73   // proportional gain
// #define _KD 46.5   // derivative gain
// #define _KI 0.0   // integral gain

#define _KP 4.76 // 4.76
#define _KD 252.9 //252.88
#define _KI 0.000002 // 0.00001
//error_current < 0 ? 0.5 : 0.5
...
unsigned long last_sampling_time_dist, last_sampling_time_servo,
    last_sampling_time_serial;               // unit: ms
...
void setup() {
    ...
    duty_chg_per_interval = (float)(_DUTY_MAX - _DUTY_MIN) *
...
void loop() {
    ...
    if (event_serial) {
        ...
        if (MODE == 0) {  // use for debugging with serial plotter
            Serial.print("MIN:0,MAX:310,TARGET:");
...
float volt_to_distance(int a_value) {
    ...
    return 1294 + -7.47 * a_value + 0.0152 * pow(a_value, 2) + -1.05E-05 *  pow(a_value, 3);
}
...
unsigned int ir_sensor_filtered(unsigned int n, float position, int verbose) {
    // Eliminate spiky noise of an IR distance sensor by repeating measurement
    // and taking a middle value n: number of measurement repetition position:
    // the percentile of the sample to be taken (0.0 <= position <= 1.0)
    // verbose: 0 - normal operation, 1 - observing the internal procedures, and
    // 2 - calculating elapsed time. Example 1: ir_sensor_filtered(n, 0.5, 0) =>
    // return the median value among the n measured samples. Example 2:
    // ir_sensor_filtered(n, 0.0, 0) => return the smallest value among the n
    // measured samples. Example 3: ir_sensor_filtered(n, 1.0, 0) => return the
    // largest value among the n measured samples.

    // The output of Sharp infrared sensor includes lots of spiky noise.
    // To eliminate such a spike, ir_sensor_filtered() performs the following
    // two steps: Step 1. Repeat measurement n times and collect n * position
    // smallest samples, where 0 <= postion <= 1. Step 2. Return the position'th
    // sample after sorting the collected samples.
    ...
    if (verbose == 1) {
        ...
        Serial.print(position);
        Serial.print(", ret_idx: ");
        Serial.println((unsigned int)(n * position));
    }

    ir_val = (unsigned int *)malloc(sizeof(unsigned int) * n);
    if (ir_val == NULL) return 0;

    if (verbose == 1) Serial.print("IR:");
